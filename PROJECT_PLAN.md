...
**Honest scope notes.** The `single_use` limit is modeled per instance (global), not per actor — "each user may redeem once" is a future refinement. Race semantics only affect the use counter; race-group members are treated in declared order for owner/exists. `state_precondition` is still `unsupported`.

**LLM race generation confirmed live (2026-09-03, run #54)** — this closes the phase's last open item. A plain `POST /runs {"target_name": "seeded-race", "replay": true}` with `LLM_ENV=dev` produced, unprompted: a **`single_use` invariant with `limit` 1 at confidence 0.95** (the highest of the three it extracted), whose rationale quotes the planted bug directly — *"checks c.redeemed then writes c.redeemed = true with no lock in between"* — and a **hypothesis marking two redeems with the same `race_group`**. Z3 proved it `sat` with the interleaving `check step 2 < check step 3 < write step 2 < write step 3`, and Stage 6 replay **confirmed** it against the running service: both redeems returned 200, released 0.288 ms and 0.077 ms after the barrier. All three of the run's hypotheses came back confirmed. Worth being precise about the causal chain here: the model only proposed the race *because* Stage 1's check-then-act evidence reached the invariant prompt — the first attempt, before that scanner fix (see Phase 8), extracted only ownership and role invariants from the same target. The evidence is what made the difference, not the prompt wording.
